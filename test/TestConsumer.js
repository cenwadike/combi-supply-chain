...
describe("SupplyChain", async function () {
  it("Should simulate a supplychain consumer", async function () {
    // deploy contract
    ...
    console.log("-------------------------------");
    console.log("SIMULATING SUPPLYCHAIN PARTIES: CONSUMER");
    console.log("-------------------------------");
    ...
    try {
      ...
      await supplyChain.connect(consumer).buyAsConsumer(1, { value: productPrice });
    ...
    try {
      await supplyChain.connect(consumer).buyAsConsumer(2, { value: distributorPrice });
      let state = await supplyChain.fetchCurrentState(2);
      expect(state).to.equal(7);
    } catch (error) {
      console.log(error);
    ...
    try {
      await supplyChain
        .connect(distributor)
        .buyAsDist(3, distributorPrice, { value: productPrice });

      let state = await supplyChain.fetchCurrentState(3);
      expect(state).to.equal(2);
    } catch (error) {
      console.log(error);
    }
    try {
      await supplyChain.connect(retailer).buyAsRetail(3, retailPrice, { value: distributorPrice });
      let state = await supplyChain.fetchCurrentState(3);
    ...
    try {
      await supplyChain.connect(consumer).buyAsConsumer(3, { value: retailPrice });
      let state = await supplyChain.fetchCurrentState(3);
      expect(state).to.equal(7);
    } catch (error) {
      console.log(error);
